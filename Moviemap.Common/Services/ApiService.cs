...
namespace Moviemap.Common.Services
{
    public class ApiService : IApiService
    {
        ...
        public async Task<Response> GetCinemaMoviesAsync<T>(string urlBase, string servicePrefix, string controller, CinemaMoviesRequest cinemaMovierequest)
        {
            try
            {
                string request = JsonConvert.SerializeObject(cinemaMovierequest);
                StringContent content = new StringContent(request, Encoding.UTF8, "application/json");
                HttpClient client = new HttpClient
                {
                    BaseAddress = new Uri(urlBase)
                ...
                var list = JsonConvert.DeserializeObject<List<MovieResponse>>(result);
                return new Response
                {
                    IsSuccess = true,
                    Result = list
